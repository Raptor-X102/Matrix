...
void test_det_with_timer_and_print(int n) {
    ...
    try {
        if constexpr (detail::is_matrix_v<ValueType>) {
            ...
            ValueType min_block = ValueType::Diagonal(sub_n, sub_n, static_cast<InnerType>(1));
            ValueType max_block = ValueType::Diagonal(sub_n, sub_n, static_cast<InnerType>(2));
            
            // Делаем min_block единичной матрицей для простоты
            min_block = ValueType::Identity(sub_n, sub_n);
            
            matrix = MatType::Generate_matrix(n, n, min_block, max_block, 5, ValueType::Identity(sub_n, sub_n));
        } else {
            // Для скалярных типов
            ...
            matrix = MatType::Generate_matrix(n, n, min_val, max_val, 10, static_cast<ValueType>(1));
        }
    ...
    if (n <= 5) {
        ...
        if constexpr (detail::is_matrix_v<ValueType>) {
            if (matrix.get_rows() > 0 && matrix.get_cols() > 0) {
                std::cout << ", block size: " << matrix(0, 0).get_rows() << "x" << matrix(0, 0).get_cols();
            }
        }
        std::cout << "\n";
    }
    ...
    if (computed_det) {
        if constexpr (detail::is_matrix_v<ValueType>) {
            std::cout << "   Determinant is a " 
                      << (*computed_det).get_rows() << "x" << (*computed_det).get_cols() 
                      << " matrix:\n";
                ...
                if ((*computed_det).get_rows() == (*cached_det).get_rows() &&
                    ...
                    for (int i = 0; i < (*computed_det).get_rows() && equal; ++i) {
                        for (int j = 0; j < (*computed_det).get_cols() && equal; ++j) {
                            if (!Matrix<ValueType>::is_equal((*computed_det)(i, j), (*cached_det)(i, j))) {
                                equal = false;
                            }
                        }
                    }
