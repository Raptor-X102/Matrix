...
struct TestConfig {
    struct TypeConfig {
        ...
        std::string inner_type =
            "double"; // для Matrix типов: "int", "double", "complex"
    };
...
template<typename T>
void test_block_vector(int size, int block_rows = 2, int block_cols = 2);

template<typename T1, typename T2> void test_mixed_types_with_T(int size1, int size2);
...
template<typename T> void test_constructors(int size) {
    std::cout << "=== Testing Vector Constructors (size=" << size
              << ") ===" << std::endl;
    ...
    Vector<T> v2(size);
    std::cout << "2. Constructor with size(" << size << "): size = " << v2.size()
              << std::endl;
    ...
    if (size <= 10)
        ...
        std::cout << "[vector too large to display]" << std::endl;
...
template<typename T> void test_arithmetic_operators(int size) {
    std::cout << "=== Testing Arithmetic Operators (size=" << size
              << ") ===" << std::endl;
...
template<typename T> void test_block_vector(int size, int block_rows, int block_cols) {
    std::cout << "=== Testing Block Vector (Vector<Matrix<" << type_name<T>()
              << ">>) ===" << std::endl;
    std::cout << "Vector size: " << size << ", Block size: " << block_rows << "x"
              << block_cols << std::endl;
...
template<typename T1, typename T2> void test_mixed_types_with_T(int size1, int size2) {
    std::cout << "=== Testing Mixed Types: " << type_name<T1>() << " + "
              << type_name<T2>() << " ===" << std::endl;
    std::cout << "Sizes: " << size1 << " and " << size2 << std::endl;

    if (size1 != size2) {
        std::cout << "Skipping - sizes must match for mixed type operations"
                  << std::endl;
        std::cout << "===================================\n" << std::endl;
        return;
    }
...
void test_avx_optimization() {
    ...
    {
        std::cout << "=== Testing AVX with float (size=8 for 256-bit registers) ==="
                  << std::endl;
        Vector<float> v1(8);
        Vector<float> v2(8);
    ...
    // Test double vectors
    {
        std::cout << "=== Testing AVX with double (size=4 for 256-bit registers) ==="
                  << std::endl;
        Vector<double> v1(4);
        Vector<double> v2(4);
...
void test_edge_cases(const TestConfig &config) {
    ...
    // Test with size 0
    {
        ...
        std::cout << "2. Empty vector begin == end: " << (empty.begin() == empty.end())
                  << std::endl;
    ...
    // Test cross product with wrong dimensions
    {
        ...
        try {
            ...
            std::cout << "1. Cross product of 2D vectors: SHOULD HAVE THROWN!"
                      << std::endl;
        } catch (const std::exception &e) {
            std::cout << "1. Cross product of 2D vectors correctly threw: " << e.what()
                      << std::endl;
        }
        ...
        try {
            auto dot = v3.dot(w4);
            std::cout << "2. Dot product of different sizes: SHOULD HAVE THROWN!"
                      << std::endl;
        } catch (const std::exception &e) {
            std::cout << "2. Dot product of different sizes correctly threw: "
                      << e.what() << std::endl;
        }
        std::cout << "===================================\n" << std::endl;
    }
...
TestConfig configure_tests() {
    ...
    if (config.int_config.enabled) {
        ...
        if (!input.empty()) {
            ...
            if (config.int_config.vector_size <= 0)
                config.int_config.vector_size = 5;
        }
    }
    ...
    if (config.double_config.enabled) {
        ...
        if (!input.empty()) {
            ...
            if (config.double_config.vector_size <= 0)
                config.double_config.vector_size = 5;
        }
    }
    ...
    if (config.complex_config.enabled) {
        ...
        if (!input.empty()) {
            ...
            if (config.complex_config.vector_size <= 0)
                config.complex_config.vector_size = 5;
        }
    }
    ...
    if (config.matrix_config.enabled) {
        ...
        if (!input.empty()) {
            ...
            if (config.matrix_config.vector_size <= 0)
                config.matrix_config.vector_size = 3;
        }
        ...
        if (!input.empty()) {
            ...
            if (config.matrix_config.block_rows <= 0)
                config.matrix_config.block_rows = 2;
        }
        ...
        if (!input.empty()) {
            ...
            if (config.matrix_config.block_cols <= 0)
                config.matrix_config.block_cols = 2;
        }
        ...
        switch (choice) {
        case 1:
            config.matrix_config.inner_type = "int";
            break;
        case 2:
            config.matrix_config.inner_type = "double";
            break;
        case 3:
            config.matrix_config.inner_type = "complex";
            break;
        default:
            config.matrix_config.inner_type = "double";
        }
...
void run_matrix_tests(const TestConfig::TypeConfig &config) {
    ...
    std::cout << "Vector size: " << config.vector_size << std::endl;
    std::cout << "Block size: " << config.block_rows << "x" << config.block_cols
              << std::endl;
    std::cout << std::string(60, '=') << "\n" << std::endl;
    ...
    if (config.inner_type == "int") {
        ...
        test_block_vector<double>(config.vector_size,
                                  config.block_rows,
                                  config.block_cols);
    } else if (config.inner_type == "complex") {
        test_block_vector<std::complex<double>>(config.vector_size,
                                                config.block_rows,
                                                config.block_cols);
    }
}
...
int main() {
    ...
    if (config.complex_config.enabled) {
        run_all_tests_for_type<std::complex<double>>("complex<double>",
