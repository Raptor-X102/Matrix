...
template<typename T> double get_element_value(const T &elem) {
    ...
    for (int i = 0; i < original.get_rows(); ++i) {
        for (int j = 0; j < original.get_cols(); ++j) {
            ...
            if (std::abs(orig_val - trans_val) > eps) {
                std::cout << "  Mismatch at [" << i << "," << j << "]: " << orig_val
                          << " != " << trans_val << "\n";
                return false;
            }
        }
...
bool verify_transpose(const MatType &original, const MatType &transposed) {
    ...
    if constexpr (detail::is_matrix_v<ValueType>) {
        return verify_transpose_block<typename ValueType::value_type>(original,
                                                                      transposed);
    } else {
...
template<typename T> bool check_block_matrix_symmetry(const Matrix<Matrix<T>> &matrix) {
...
template<typename MatType> void test_transpose_with_timing(int rows, int cols) {
    using ValueType = typename MatType::value_type;
    ...
    try {
        ...
        std::cout << "   Average time over " << iterations
                  << " iterations: " << std::fixed << std::setprecision(9) << avg_time
                  << " s (" << format_duration(avg_time) << ")\n";
        ...
        if (correct) {
            std::cout << "      ✓ (A^T)^T == A (verification took " << elapsed.count()
                      << " s)\n";
        } else {
            std::cout << "      ✗ (A^T)^T != A\n";
        }
        ...
        if (rows == cols) {
            ...
            if constexpr (detail::is_matrix_v<ValueType>) {
                ...
                bool symmetric_correct =
                    check_block_matrix_symmetry<typename ValueType::value_type>(
                        symmetric);
        ...
        try {
            if (type_choice == "1") {
                ...
                if (inner_type == "1") {
                    ...
                    test_transpose_with_timing<Matrix<Matrix<std::complex<double>>>>(
                        rows,
                        cols);
                } else {
