...
template<typename MatType> double check_matrix_identity(const MatType &M) {
            ...
            if constexpr (detail::is_matrix_v<ValueType>) {
                ...
                for (int bi = 0; bi < block.get_rows(); ++bi) {
                    for (int bj = 0; bj < block.get_cols(); ++bj) {
                        ...
                        if constexpr (std::is_same_v<typename ValueType::value_type,
                                                     std::complex<double>>) {
...
template<typename MatType> void test_inverse_matrix(int n) {
    using ValueType = typename MatType::value_type;
    ...
    try {
        ...
        if constexpr (detail::is_matrix_v<ValueType>) {
                ...
                for (int j = 0; j < n; ++j) {
                    if (i == j) {
                        ...
                        for (int bi = 0; bi < block_size; ++bi) {
                            ...
                            if constexpr (std::is_same_v<InnerType,
                                                         std::complex<double>>) {
            ...
            do {
                ...
                if (attempts > 5) {
                    std::cout << "   ⚠ Could not generate invertible matrix after "
                              << attempts << " attempts\n";
                    ...
                    for (int i = 0; i < n; ++i) {
                        ...
                        if (i > 0)
                            A(i, i - 1) = static_cast<ValueType>(0.1);
                        if (i < n - 1)
                            A(i, i + 1) = static_cast<ValueType>(0.1);
                    }
                    break;
        ...
        if (det_opt) {
            std::cout << "   det(A) ";
            if constexpr (detail::is_matrix_v<ValueType>) {
                std::cout << "is " << (*det_opt).get_rows() << "x"
                          << (*det_opt).get_cols() << " matrix\n";
