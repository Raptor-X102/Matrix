...
private:
    ...
    static std::vector<T> create_controlled_diagonal(int size,
        ...
        if constexpr (std::is_integral_v<T>) {
            ...
            T partial_product = T{1};
            for (int i = 0; i < size - 1; ++i) {
                partial_product *= diagonal[i];
            }
            
            if (partial_product != T{0}) {
                T desired_last = target_determinant_magnitude;
                
                T best_last = T{1};
                T best_diff = std::abs(target_determinant_magnitude - partial_product);
                
                for (T test_val = min_val; test_val <= max_val; ++test_val) {
                    if (test_val == T{0}) continue;
                    
                    T test_product = partial_product * test_val;
    ...
    static void apply_transformation_type_II(Matrix &matrix, int rows, int cols,
        std::conditional_t<std::is_same_v<T, int>, int, T> min_val,
        std::conditional_t<std::is_same_v<T, int>, int, T> max_val, T &effective_det) {
        ...
        if (!matrix.is_zero(scalar)) {
            if constexpr (std::is_floating_point_v<T>) {
                ...
                if (std::abs(scalar) > T{100}) {
                    scalar = std::copysign(T{100}, scalar);
                }
            }
...
public:
    ...
    static Matrix Generate_matrix(int rows, int cols,
        ...
        std::conditional_t<std::is_same_v<T, int>, int, T> max_condition_number = {}) {
