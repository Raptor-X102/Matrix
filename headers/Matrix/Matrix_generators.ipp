...
Matrix<T> Matrix<T>::Generate_matrix(int rows,
    ...
    if constexpr (detail::is_matrix_v<T>) {
        return generate_block_matrix(rows, cols, min_val, max_val, iterations,
                                     target_determinant_magnitude, max_condition_number);
    } else {
        return generate_scalar_matrix(rows, cols, min_val, max_val, iterations,
                                      target_determinant_magnitude, max_condition_number);
    }
}
...
Matrix<T> Matrix<T>::Generate_binary_matrix(int rows,
    ...
    try {
        return generate_binary_matrix_impl(rows, cols, target_determinant_magnitude, iterations);
    } catch (const std::exception& e) {
        throw std::runtime_error(std::string("Failed to generate binary matrix: ") + e.what());
    }
}
...
template<typename T>
void Matrix<T>::validate_generation_parameters(T& min_val, T& max_val,
...
template<typename T>
std::vector<T> Matrix<T>::generate_binary_diagonal(int size) {
...
template<typename T>
Matrix<T> Matrix<T>::create_initial_binary_matrix(int rows, int cols,
...
template<typename T>
void Matrix<T>::apply_random_binary_transformation(Matrix& matrix, int rows, int cols,
...
template<typename T>
void Matrix<T>::apply_binary_transformation_II(Matrix& matrix, int rows, T& effective_det) {
...
std::vector<T> Matrix<T>::create_controlled_diagonal(int size,
    ...
    if constexpr (std::is_integral_v<T>) {
        return create_integer_diagonal(size, min_val, max_val, target_determinant_magnitude);
    } else {
        return create_numeric_diagonal(size, min_val, max_val, target_determinant_magnitude);
    }
}
...
std::vector<T> Matrix<T>::create_integer_diagonal(int size,
    ...
    if (current_product != T{0}) {
        diagonal[size - 1] = find_best_last_element(current_product, min_val, max_val,
                                                    target_determinant_magnitude);
    } else {
...
std::vector<T> Matrix<T>::create_numeric_diagonal(int size,
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (i == size - 1 && current_product != T{0}) {
            element = compute_last_element(current_product, target_determinant_magnitude);
...
template<typename T>
T Matrix<T>::find_best_last_element(T current_product, T min_val, T max_val,
                                    T target_determinant_magnitude) {
    T best_element = T{1};
...
template<typename T>
T Matrix<T>::generate_nonzero_random(T min_val, T max_val) {
    T element;
    do {
...
template<typename T>
T Matrix<T>::normalize_element(T element) {
...
template<typename T>
T Matrix<T>::clamp_extreme_value(T value) {
...
void Matrix<T>::apply_controlled_transformations(Matrix &matrix,
    ...
    for (int i = 0; i < iterations; ++i) {
        apply_single_transformation(matrix, rows, cols, min_val, max_val,
                                   effective_det, sign);
    }
...
void Matrix<T>::apply_single_transformation(Matrix &matrix,
    ...
    try {
        switch (transformation) {
            ...
            case Tranformation_types::II_TYPE:
                apply_type_II_transformation(matrix, rows, cols, min_val, max_val,
                                            effective_det);
                break;
...
void Matrix<T>::apply_type_II_transformation(Matrix &matrix,
    ...
    if constexpr (std::is_same_v<T, int>) {
        ...
        if (is_safe_for_integer_multiplication(matrix, row, cols, scalar, min_val, max_val)) {
        ...
        if constexpr (detail::is_ordered_v<T>) {
            ...
            if (is_safe_for_numeric_multiplication(matrix, row, cols, scalar, min_val, max_val)) {
...
template<typename T>
T Matrix<T>::clamp_integer_scalar(T scalar) {
...
template<typename T>
T Matrix<T>::clamp_numeric_scalar(T scalar) {
    if constexpr (std::is_same_v<T, float>) {
...
template<typename T>
T Matrix<T>::clamp_transformation_scalar(T scalar) {
    if constexpr (std::is_same_v<T, int>) {
...
template<typename T>
bool Matrix<T>::needs_stabilization(T effective_det) {
    if constexpr (detail::is_ordered_v<T>) {
        if constexpr (std::is_same_v<T, float>) {
...
void Matrix<T>::apply_transformation_type_II(Matrix &matrix,
                                             ...
                                             T max_val,
                                             T &effective_det) {
    apply_type_II_transformation(matrix, rows, cols, min_val, max_val, effective_det);
}
